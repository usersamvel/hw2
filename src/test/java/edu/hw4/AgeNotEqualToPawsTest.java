...
class AgeNotEqualToPawsTest {

    @Test
    void ageNotEqualToPaws() {
        AgeNotEqualToPaws ageNotEqualToPaws = new AgeNotEqualToPaws();
        ...
        response.add(A);
        response.add(B);
        Assertions.assertThat(ageNotEqualToPaws.ageNotEqualToPaws(list)).isEqualTo(response);
    }

    @Test
    @DisplayName("Пустой список")
    void ageNotEqualToPaws2() {
        AgeNotEqualToPaws ageNotEqualToPaws = new AgeNotEqualToPaws();
        Animal A = new Animal("A", Animal.Type.CAT, Animal.Sex.M, 12, 132, 50, true);
        Animal B = new Animal("B", Animal.Type.CAT, Animal.Sex.M, 20, 190, 70, true);
        Animal C = new Animal("C", Animal.Type.DOG, Animal.Sex.F, 4, 100, 30, true);
